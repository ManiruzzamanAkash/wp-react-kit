<form <?php echo wp_kses_data( get_block_wrapper_attributes( [ 'class' => 'jobplace-jobs-search' ] ) ); ?> data-wp-on--submit="actions.submitSearch">
	...
	<input id="jobplace-jobs-search-input" class="jobplace-jobs-search__input" type="search" placeholder="<?php echo esc_attr( $attributes['placeholder'] ?? __( 'Search jobs…', 'jobplace' ) ); ?>" data-wp-bind--value="state.search" data-wp-on--input="actions.updateSearch" data-wp-on--keydown="actions.handleSearchKeydown" />
